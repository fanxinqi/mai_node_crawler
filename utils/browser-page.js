...
class BrowerPage {
   ...
   * }
   */
  async open(url, selecters, options = { n: 0, retryTime: 20 }) {
    if (!this.browser) {
    ...
    let result;
    try {
      result = await this.waitForSelecter(selecters, options);
    } catch (error) {
      log.error(error);
      return null;
    }
